# -*- coding: utf-8 -*-
"""Jayanthi_RGB_3.0ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14Nt-jnyaYgDF-8_IYB2uQgOqL4brF0X_
...
# ======================== SIMPLE CONFIG ========================
GENERATION_MODEL = "llama-3.3-70b-versatile"  # Groq model for generation
JUDGMENT_MODEL = "deepseek-r1-distill-llama-70b" #"llama-3.3-70b-versatile"  # DeepSeek model through Groq for judgment
Temperature = 0.2
...
temperature_list = [0.6]
noise_rate_list = [ 0.0,0.2,0.4,0.6, 0.8] #0.0,0.2,0.4,
...
# def predict_negative_rejection(query, ground_truth, docs, model, system, instruction, temperature):
#     """
#     Generate prediction using the RGB instruction format.
#     """
#     prediction = ""
#     try:
...
#     """
#     RGB Paper methodology for negative rejection:
...
#     """
#     Check if the prediction is an exact rejection
#     """
#     # The exact phrase from the RGB paper instruction
#     exact_phrase = "I can not answer the question because of the insufficient information in documents."
#     return exact_phrase in prediction

# def evaluate_rejection_with_llm(query, prediction, api_key, judgment_model_name):
#     """
#     Evaluate rejection using an LLM (Rej* metric).
#     The LLM determines if the response indicates inability to answer based on docs.
#     """
#     # Define a prompt for the judgment model
...
# """
#     try:
#         judge_model = GroqModel(api_key=api_key, plm=judgment_model_name)
#         evaluation = judge_model.generate(text=judge_prompt, temperature=0.0, max_new_tokens=10) # Use low temp for consistent output
#         # Check if the evaluation starts with "YES" (case-insensitive)
#         return evaluation.strip().upper().startswith("YES")
#     except Exception as e:
#         print(f"Error during LLM evaluation: {str(e)}")
#         return False # Assume it didn't indicate rejection if evaluation fails


# # ======================== RGB TABLE 3 REPLICATION ========================
# def run_rgb_negative_rejection_english(
#     dataset="en",
#     model_name="Groq",
#     temperature=Temperature,
#     passage_num=5,
#     num_samples=300  # RGB paper uses full dataset, we'll use 300 samples
# ):
#     """
#     Replicate RGB Paper Table 3 methodology for English
#     Generate both Rej and Rej* metrics exactly as in the paper
#     """
#     print("📋 RGB PAPER TABLE 3 REPLICATION - NEGATIVE REJECTION (ENGLISH)")
#     print(f"Model: {model_name}")
#     print(f"Testing methodology:")
#     print(f"  - All documents are IRRELEVANT (negative documents only)")
#     print(f"  - Documents per question: {passage_num}")
#     print(f"  - Samples: {num_samples}")
#     print(f"  - Rej: Exact phrase matching")
#     print(f"  - Rej*: LLM-based evaluation")
#     print("-" * 70)
...
#         instances = [json.loads(line) for line in f]
#     print(f"Loaded {len(instances)} instances from {dataset_file}")

#     # Load RGB instruction format
#     with open('/content/instruction.yaml', 'r') as f:
#         prompt = yaml.safe_load(f)['en']  # English only
...
#     generation_model = GroqModel(api_key=api_key, plm=GENERATION_MODEL)
...
#     os.makedirs(f"/content/{resultpath}", exist_ok=True)
#     filename = f"/content/{resultpath}/rgb_negative_rejection_{model_name}_temp{temperature}.json"
...
#     # Ensure num_samples doesn't exceed available instances
#     test_instances = instances[:min(num_samples, len(instances))]
#     print(f"Processing {len(test_instances)} samples...")


#     with open(filename, 'w') as f:
#         for instance in tqdm.tqdm(test_instances, desc="Processing"):
#             try:
#                 random.seed(2333)  # RGB paper uses fixed seed
#                 query, ans, docs = processdata_negative_rejection(instance, passage_num)

#                 # Generate prediction
#                 prediction = predict_negative_rejection(query, ans, docs, generation_model, system, instruction, temperature)

#                 # Rej: Exact phrase matching (RGB methodology)
#                 exact_rejection = check_exact_rejection(prediction)
#                 if exact_rejection:
#                     exact_rejections += 1

#                 # Rej*: LLM evaluation (RGB methodology)
#                 # Pass the judgment model name to the evaluation function
...
#     # Print results in RGB Table 3 format
#     print(f"\n📊 RESULTS (RGB Table 3 Format):")
#     print("-" * 50)
#     print("Languages    | English")
#     print("             | Rej    | Rej*")
#     print("-" * 50)
#     print(f"{model_name:12} | {rej_rate:6.2f} | {rej_star_rate:5.2f}")
#     print("-" * 50)

#     print(f"\n📈 DETAILED BREAKDOWN:")
#     print(f"Total samples: {total}")
#     print(f"Exact rejections (Rej): {exact_rejections} ({rej_rate:.2f}%)")
#     print(f"LLM-judged rejections (Rej*): {llm_rejections} ({rej_star_rate:.2f}%)")

#     # Show some examples
#     print(f"\n📝 SAMPLE PREDICTIONS:")
#     print("-" * 60)
#     # Ensure we don't try to print more examples than available results
#     num_examples_to_print = min(3, len(results))
#     for i, result in enumerate(results[:num_examples_to_print]):
#         status_exact = "✅ EXACT REJ" if result['exact_rejection'] else "❌ EXACT"
#         status_llm = "✅ LLM REJ" if result['llm_rejection'] else "❌ LLM"
#         print(f"\nExample {i+1}: [{status_exact}] [{status_llm}]")
#         print(f"Q: {result['query'][:70]}...")
#         print(f"A: {result['prediction'][:100]}...")

#     print(f"\n📁 Files saved:")
#     print(f"  - Predictions: {filename}")
#     print(f"  - Scores: {result_file}")

#     return scores

# # ======================== COMPARISON WITH RGB PAPER RESULTS ========================
# def compare_with_rgb_paper(our_results):
#     """Compare our results with RGB paper Table 3"""
#     print(f"\n📊 COMPARISON WITH RGB PAPER TABLE 3:")
#     print("-" * 60)

#     # RGB Paper results for English (from the table image)
#     rgb_paper_results = {
#         "ChatGPT": {"rej": 24.67, "rej_star": 45.00},
#         "ChatGLM-6B": {"rej": 9.00, "rej_star": 25.00},
#         "ChatGLM2-6B": {"rej": 10.33, "rej_star": 41.33},
#         "Vicuna-7B-v1.3": {"rej": 17.00, "rej_star": 33.33},
#         "Qwen-7B-Chat": {"rej": 31.00, "rej_star": 35.67},
#         "BELLE-7B-2M": {"rej": 5.67, "rej_star": 32.33}
#     }

#     print("Model           | RGB Paper      | Our Results")
#     print("                | Rej   | Rej*   | Rej   | Rej*")
#     print("-" * 50)

#     # Show RGB paper results
#     for model, scores in rgb_paper_results.items():
#         print(f"{model:15} | {scores['rej']:5.2f} | {scores['rej_star']:5.2f} |   -   |   -")

#     # Show our results
#     print(f"{'Our Groq':15} |   -   |   -   | {our_results['rej_rate']:5.2f} | {our_results['rej_star_rate']:5.2f}")

#     print(f"\nℹ️  Key Insights from RGB Paper:")
#     print(f"  - Highest Rej rate: {max(v['rej'] for v in rgb_paper_results.values()):.2f}% ({max(rgb_paper_results, key=lambda k: rgb_paper_results[k]['rej'])})")
#     print(f"  - Highest Rej* rate: {max(v['rej_star'] for v in rgb_paper_results.values()):.2f}% ({max(rgb_paper_results, key=lambda k: rgb_paper_results[k]['rej_star'])})")
#     print(f"  - Our Groq result: Rej={our_results['rej_rate']:.2f}%, Rej*={our_results['rej_star_rate']:.2f}%")


# # ======================== RUN RGB REPLICATION ========================
# if __name__ == "__main__":
#     # Run the exact RGB Table 3 methodology
#     results = run_rgb_negative_rejection_english(
#         dataset="en_refine",
#         model_name="Groq",
#         temperature=Temperature,
#         passage_num=5,
...
#     compare_with_rgb_paper(results)
...
def run_complete_factcheck():
    ...
    if scores and df is not None:
        # Additional analysis
        with open(f"/content/result-en/fact/prediction_en_fact_Groq_temp0.7_noise0.0_passage5_correct0.0.json", 'r') as f:
...
models_data = {
    "llama-3.3-70b-versatile": [99.66, 99.66, 100, 99.66, 96.66],
    "llama-3.1-8b-instant": [98.7, 99.3, 99, 98.7, 92.7],
    "gemma2-9b-it": [98.7, 99.3, 99.3, 98, 96],
    "qwen/qwen3-32b": [100, 100, 100, 100, 98.7],
    "deepseek-r1-distill-llama-70b": [100, 100, 100, 99.3, 98.3]
}

# Plotting
plt.figure(figsize=(9, 7))
for model, accuracies in models_data.items():
    plt.plot(noise_levels, accuracies, marker='o', label=model)
...
# Use seaborn's theme for aesthetics
...
models_data = {
    "LLaMA 3.3 70B (Versatile)": [99.66, 99.66, 100, 99.66, 96.66],
    "LLaMA 3.1 8B (Instant)": [98.7, 99.3, 99, 98.7, 92.7],
    "Gemma 2 9B IT": [98.7, 99.3, 99.3, 98, 96],
    "Qwen 3 32B": [100, 100, 100, 100, 98.7],
    "Deepseek-R1 Distill LLaMA 70B": [100, 100, 100, 99.3, 98.3]
}

# Set color palette
palette = sns.color_palette("husl", n_colors=len(models_data))

# Plotting
plt.figure(figsize=(12, 5))

for (model, accuracies), color in zip(models_data.items(), palette):
    plt.plot(noise_levels, accuracies, marker='o', label=model, linewidth=1.5, markersize=5, color=color)

plt.title("💡 Model Accuracy vs. Noise Level (RGB Task)", fontsize=18, weight='bold')
...
models_data = {
    "llama-3.3-70b-versatile": [87, 74, 63],
    "llama-3.1-8b-instant": [84, 82, 75],
    "gemma2-9b-it": [89, 85, 77],
    "qwen/qwen3-32b": [89, 83, 75],
    "deepseek-r1-distill-llama-70b": [92, 87, 79]
}

# Plotting
plt.figure(figsize=(8, 5))
for model, scores in models_data.items():
    plt.plot(noise_levels, scores, marker='o', label=model, linewidth=1.2, markersize=4)

# Labels and style adjustments
#plt.title("Information Integration vs Noise Level", fontsize=10)
plt.xlabel("Noise Level", fontsize=9)
plt.ylabel("Integration Score (%)", fontsize=9)
...
# Data
models = ["llama-3.1-8b-instant", "gemma2-9b-it", "qwen/qwen3-32b", "deepseek-r1-distill-llama-70b"]
rej = [67.33, 66.0, 13.33, 1.33]
rej_star = [75.67, 70.67, 67.33, 63.0]
...
# Set up figure
plt.figure(figsize=(8, 5))
bars1 = plt.bar(x - width/2, rej, width, label='Rej', color='#aec7e8')
bars2 = plt.bar(x + width/2, rej_star, width, label='Rej*', color='#ffbb78')

# Labels and title
plt.ylabel("Rejection Rate (%)", fontsize=10)
#plt.title("LLM Rejection Rate Comparison", fontsize=12)
plt.xticks(x, models, rotation=15, fontsize=8)
plt.yticks(fontsize=8)
plt.ylim(-1, 85)
...
for bar in bars1 + bars2:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 1, f"{yval:.1f}", ha='center', va='bottom', fontsize=7)
...
# Models and metrics
models = ["llama-3.3-70b-versatile", "llama-3.1-8b-instant", "gemma2-9b-it", "qwen/qwen3-32b"]
accuracy = [91, 77, 53, 76]
error_detection = [44, 53, 62, 88]
error_correction = [79.5, 79.2, 58.1, 76.1]
...
plt.bar(x - width, accuracy, width, label='Accuracy with Docs (%)', color='#aec7e8')
plt.bar(x, error_detection, width, label='Error Detection Rate (%)', color='#ffbb78')
plt.bar(x + width, error_correction, width, label='Error Correction Rate (%)', color='#98df8a')

# Axis formatting
plt.ylabel("Percentage (%)", fontsize=10)
#plt.title("LLM Performance on Document-Based Tasks", fontsize=12)
plt.xticks(x, models, rotation=15, fontsize=8)
plt.yticks(fontsize=8)
plt.ylim(0, 100)
plt.legend(fontsize=9)
...
def add_labels(values, offset):
    for i, v in enumerate(values):
        plt.text(x[i] + offset, v + 1, f"{v:.1f}", ha='center', fontsize=7)

add_labels(accuracy, -width)
add_labels(error_detection, 0)
add_labels(error_correction, width)
